...
class Home(tk.Frame):
    ...
    def __init__(self,parent,root,user_name):
        ...
        self.recieve = tk.Button(self,text="Recieve",relief=tk.GROOVE,fg='red',bg='orange',font=("arial",16,"bold"),command=print)
        self.recieve.pack(fill=tk.BOTH,padx=2,pady=10)


        self.settings_icon = tk.PhotoImage(file ="setting.png")
        self.settings = tk.Button(self, image = self.settings_icon,bg='black')
        self.settings.pack(side = tk.LEFT,padx=20)

        self.about_icon = tk.PhotoImage(file = "about.png")
